...
Number of valid data points for calibration and evaluation:
```{r}
...
load("~/eval_pmodel/data/ddf_obs_calib_NT.Rdata")
ddf_obs_calib %>% 
...
filn <- "~/eval_pmodel/data/obs_eval_NT_WITHFORCING_FPARspl.Rdata"
...
Collect calibrated parameter values.
```{r}
df_params <- read_csv("~/eval_pmodel/calib_results/params_opt_FULL.csv") %>%        bind_cols(tibble(setup = "FULL"), .)  %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_BRC.csv") %>%          bind_cols(tibble(setup = "BRC"), .)) %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_ORG.csv") %>%          bind_cols(tibble(setup = "ORG"), .)) %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_FULL_FPARitp.csv") %>% bind_cols(tibble(setup = "FULL_FPARitp"), .)) %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_FULL_EVI.csv") %>%     bind_cols(tibble(setup = "FULL_EVI"), .)) %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_FULL_DT.csv") %>%      bind_cols(tibble(setup = "FULL_DT"), .)) %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_FULL_NT.csv") %>%      bind_cols(tibble(setup = "FULL_NTsub"), .)) %>% 
  bind_rows(read_csv("~/eval_pmodel/calib_results/params_opt_FULL_Ty.csv") %>%      bind_cols(tibble(setup = "FULL_Ty"), .)) %>% 
  mutate_all(format, digits = 5)
df_params
```
...
df_dday_agg %>% 
  ...
  geom_line(
    aes(x = dday, y = bias_gpp_median, color = setup), size = 0.9) +
  geom_ribbon(
...
## Greenness data: Mean seasonal cycle (Fig. 6)
...
## Flux decomposition method (Fig. 7)
...
## SCATTER PLOT
  ...
  gg_modobs_NT <- modobs_xdf_NT$gg +
    labs(x = expression( paste("simulated GPP (g C m"^-2, "d"^-1, ")" ) ), 
         y = expression( paste("observed GPP (g C m"^-2, "d"^-1, ")" ) ),
         title = "NT") +
    xlim(0, 20) +
    ylim(0, 20)
...
## arrange and save
cowplot::plot_grid(gg_NT, gg_DT, gg_Ty, gg_modobs_NT, gg_modobs_DT, gg_modobs_Ty, nrow = 2, labels = c("(a)", "(b)", "(c)", "(d)", "(e)", "(f)"), label_size = 12, rel_heights = c(1, 1.2))
...
ddf <- out_eval_FULL$gpp$fluxnet2015$data$idvdf %>% 
  dplyr::rename(gpp_obs = obs, gpp_mod = mod) %>% 
  tidyr::drop_na(gpp_obs) %>% 
  dplyr::select(gpp_mod, gpp_obs) %>%
...
gg_ddf <- ddf %>% 
  ...
  annotate( 
    ...
    label = paste("sigma[obs] == ", 
                  format( sd(out_eval_FULL$gpp$fluxnet2015$data$idvdf$obs, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
  annotate( 
    "text", x = 5, y = 0.76, 
    label = paste("sigma[mod] == ", 
                  format( sd(out_eval_FULL$gpp$fluxnet2015$data$idvdf$mod, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
...
gg_xdf <- xdf %>% 
  ...
  annotate( 
    "text", x = 4, y = 0.8, 
    label = paste("sigma[obs] == ", 
                  format( sd(out_eval_FULL$gpp$fluxnet2015$data$ixvdf$obs, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
  annotate( 
    "text", x = 4, y = 0.76, 
    label = paste("sigma[mod] == ", 
                  format( sd(out_eval_FULL$gpp$fluxnet2015$data$ixvdf$mod, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
  xlim(-10,10) + ylim(0, 0.8) +
...
if (!exists("out_oob_ORG")){ 
  load("./calib_results/out_oob_ORG.Rdata")
}
...
df_params_oob_ORG <- extract_params_oob(param = "kphio", path = "~/eval_pmodel/calib_results/oob_ORG/", sitenames = df_metrics_oob_ORG$sitename)
  
# get calibrated parameters from single-site calibration
df_params_single_ORG <- read_csv("./calib_results/params_opt_ORG.csv")
...
df_metrics_oob_ORG %>% 
  ...
  geom_vline(xintercept = mean(df_metrics_oob_ORG$rsq), col = "red", linetype = "dashed") + 
  theme_classic() +
...
df_metrics_oob_ORG %>% 
  ...
  geom_vline(xintercept = mean(df_metrics_oob_ORG$rmse), col = "red", linetype = "dashed") + 
  theme_classic() +
...
if (!exists("out_oob_FULL")){ 
  load("./calib_results/out_oob_FULL.Rdata") 
}

## for mlflx dslab course
df_metrics_oob <- out_oob_FULL %>% 
  purrr::map("gpp") %>% 
  purrr::map("fluxnet2015") %>% 
  purrr::map("metrics") %>% 
  purrr::map("daily_pooled") %>% 
  bind_rows(.id = "site")

write_csv(df_metrics_oob, file = "data/df_metrics_oob_stocker20gmd_fig2.csv")

## for mlflx dslab course
df_metrics_oob %>% 
  ggplot(aes(x = rsq, y = ..count..)) +
  geom_histogram(color = "black", alpha = 0.3, binwidth = 0.05, position="identity") +
  theme_classic() +
  labs(x = bquote(italic(R)^2), y = "Count", title = "Out-of-sample evaluation of daily GPP",
       subtitle = "Stocker et al., 2020 GMD")
ggsave("fig/rsq_oob_stocker20gmd_fig2.pdf", width = 4, height = 4)
  
## extract metrics from individual left-out calibrations/evaluations
df_metrics_oob_FULL <- extract_metric_oob(out_oob_FULL, metric = "rsq") %>% 
  left_join(extract_metric_oob(out_oob_FULL, metric = "rmse"), by = "sitename")

# get parameters from individual left-out calibrations
df_params_oob_FULL <- extract_params_oob(param = "kphio", path = "~/eval_pmodel/calib_results/oob_FULL/", sitenames = df_metrics_oob_FULL$sitename)
  
# get calibrated parameters from single-site calibration
df_params_single_FULL <- read_csv("./calib_results/params_opt_FULL.csv")
...
## kphio parameter
gg_kphio <- df_params_oob_FULL %>% 
  ggplot() +
  geom_histogram(aes(x=kphio, y = ..count..),
    color = "black", alpha = 0.3, binwidth = 0.0001, 
    position="identity") +
  geom_vline(xintercept = df_params_single_FULL$kphio, col = "red") +
  geom_vline(aes(xintercept = mean(df_params_oob_FULL$kphio)), col = "red", linetype = "dashed") +  # Mean across out-of-bag evaluations
  theme_classic() +
  labs(x = latex2exp::TeX("$\\widehat{c_L} \\;(mol\\; mol^{-1})$"), y = "Count")  # bquote(varphi[0])

## a-parameter
gg_apar <- df_params_oob_FULL %>% 
  ggplot() +
  geom_histogram(aes(x=soilm_par_a, y = ..count..),
    color = "black", alpha = 0.3, binwidth = 0.01, 
    position="identity") +
  geom_vline(xintercept = df_params_single_FULL$soilm_par_a, col = "red") +
  geom_vline(aes(xintercept = mean(df_params_oob_FULL$soilm_par_a)), col = "red", linetype = "dashed") +  # Mean across out-of-bag evaluations
  theme_classic() +
  labs(x = latex2exp::TeX("$\\widehat{a_\\theta}$"), y = "Count") +  # bquote(varphi[0])
  xlim(x = c(-0.01, 0.1))

## b-parameter
gg_bpar <- df_params_oob_FULL %>% 
  ggplot() +
  geom_histogram(aes(x=soilm_par_b, y = ..count..),
    color = "black", alpha = 0.3, binwidth = 0.01, 
    position="identity") +
  geom_vline(xintercept = df_params_single_FULL$soilm_par_b, col = "red") +
  geom_vline(aes(xintercept = mean(df_params_oob_FULL$soilm_par_b)), col = "red", linetype = "dashed") +  # Mean across out-of-bag evaluations
  theme_classic() +
  labs(x = latex2exp::TeX("$\\widehat{b_\\theta}$"), y = "Count")  # bquote(varphi[0])

## R2
gg_rsq <- df_metrics_oob_FULL %>% 
  ggplot() +
  geom_histogram(aes(x = rsq, y = ..count..),
    color = "black", alpha = 0.3, binwidth = 0.05, 
    position="identity") +
  geom_vline(aes(xintercept = out_eval_FULL$gpp$fluxnet2015$metrics$xdaily_pooled$rsq), col = "red") +  # All sites simultaneously
  geom_vline(aes(xintercept = mean(df_metrics_oob_FULL$rsq)), col = "red", linetype = "dashed") +  # Mean across out-of-bag evaluations
  theme_classic() +
  labs(x = bquote(italic(R)^2), y = "Count")
...
## RMSE
gg_rmse <- df_metrics_oob_FULL %>% 
  ggplot() +
  geom_histogram(aes(x = rmse, y = ..count..),
    color = "black", alpha = 0.3, binwidth = 0.5, 
    position="identity") +
  geom_vline(aes(xintercept = out_eval_FULL$gpp$fluxnet2015$metrics$xdaily_pooled$rmse, linetype = "all"), col = "red") +
  geom_vline(aes(xintercept = mean(df_metrics_oob_FULL$rmse), linetype = "mean"), col = "red") + 
  theme_classic() +
  labs(x = expression( paste("RMSE (g C m"^-2, " d"^-1, ")" ) ), y = "Count") +
  scale_linetype_manual(name = "", values = c(all = "solid", mean = "dashed"), labels = c(all = "All sites simultaneously", mean = "Mean across out-of-bag evaluations"))
  # theme(legend.position = c(0.6,0.85))

toprow <- plot_grid(gg_apar, gg_bpar, gg_kphio, labels = "auto", ncol = 3)
bottomrow <- plot_grid(gg_rsq, gg_rmse, labels = c("d", "e"), ncol = 2, rel_widths = c(1,2))

cowplot::plot_grid(toprow, bottomrow, nrow = 2)
ggsave("./fig/oob_grid.pdf", width = 8, height = 5)
```
...
metainfo_Tier1_sites_kgclimate_fluxnet2015 %>% 
  filter(sitename %in% settings_eval$sitenames)
...
```{r}
out_eval_ORG$gpp$fluxnet2015$data$ddf %>% 
  dplyr::select(sitename, date, gpp = mod) %>% 
  mutate(setup = "ORG") %>% 
  bind_rows(
    out_eval_BRC$gpp$fluxnet2015$data$ddf %>% 
    dplyr::select(sitename, date, gpp = mod) %>% 
    mutate(setup = "BRC")
  ) %>% 
  bind_rows(
  ...
  write_csv(path = "./data_public/gpp_pmodel_fluxnet2015_stocker19gmd_daily.csv")

out_eval_ORG$gpp$fluxnet2015$data$xdf %>% 
  dplyr::select(sitename, date_start = inbin, gpp = mod) %>% 
  mutate(setup = "ORG") %>% 
  bind_rows(
    out_eval_BRC$gpp$fluxnet2015$data$xdf %>% 
    dplyr::select(sitename, date_start = inbin, gpp = mod) %>% 
    mutate(setup = "BRC")
  ) %>% 
  bind_rows(
  ...
  write_csv(path = "./data_public/gpp_pmodel_fluxnet2015_stocker19gmd_8daily.csv")

out_eval_ORG$gpp$fluxnet2015$data$adf %>% 
  dplyr::select(sitename, gpp = mod, date) %>% 
  mutate(setup = "ORG", year = lubridate::year(date)) %>%
  dplyr::select(-date) %>% 
  bind_rows(
    out_eval_BRC$gpp$fluxnet2015$data$adf %>% 
    dplyr::select(sitename, gpp = mod, date) %>% 
    mutate(setup = "BRC", year = lubridate::year(date)) %>%
    dplyr::select(-date)
  ) %>% 
  bind_rows(
    out_eval_FULL$gpp$fluxnet2015$data$adf %>% 
    dplyr::select(sitename, gpp = mod, date) %>% 
    mutate(setup = "FULL", year = lubridate::year(date)) %>%
    dplyr::select(-date) 
  ) %>% 
  write_csv(path = "./data_public/gpp_pmodel_fluxnet2015_stocker19gmd_annual.csv")

out_eval_ORG$gpp$fluxnet2015$data$meandoydf %>% 
  dplyr::select(sitename, doy, gpp = mod_mean) %>% 
  mutate(setup = "ORG") %>% 
  bind_rows(
    out_eval_BRC$gpp$fluxnet2015$data$meandoydf %>% 
    dplyr::select(sitename, doy, gpp = mod_mean) %>% 
    mutate(setup = "BRC")
  ) %>% 
  bind_rows(
    out_eval_FULL$gpp$fluxnet2015$data$meandoydf %>% 
    dplyr::select(sitename, doy, gpp = mod_mean) %>% 
    mutate(setup = "FULL") 
  ) %>% 
  write_csv(path = "./data_public/gpp_pmodel_fluxnet2015_stocker19gmd_meanseason.csv")

out_eval_ORG$gpp$fluxnet2015$data$meandf %>% 
  dplyr::select(sitename, gpp = mod) %>% 
  mutate(setup = "ORG") %>% 
  bind_rows(
    out_eval_BRC$gpp$fluxnet2015$data$meandf %>% 
    dplyr::select(sitename, gpp = mod) %>% 
    mutate(setup = "BRC")
  ) %>% 
  bind_rows(
    out_eval_FULL$gpp$fluxnet2015$data$meandf %>% 
    dplyr::select(sitename, gpp = mod) %>% 
    mutate(setup = "FULL") 
  ) %>% 
  write_csv(path = "./data_public/gpp_pmodel_fluxnet2015_stocker19gmd_spatial.csv")

```

## Data for sharing in group
...
load("~/eval_pmodel/data/obs_eval_NT_WITHFORCING_FPARitpl.Rdata")
obs_eval_itpl <- obs_eval_NT

load("~/eval_pmodel/data/obs_eval_NT_WITHFORCING_FPARspl.Rdata")
obs_eval_spl <- obs_eval_NT

load("~/eval_pmodel/calib_results/out_eval_FULL_EVI.Rdata")
xxxxx

ddf <- out_eval_FULL$gpp$fluxnet2015$data$ddf %>% 
  rename(gpp_mod_FULL = mod, gpp_obs = obs) %>% 
  left_join(dplyr::select(out_eval_ORG$gpp$fluxnet2015$data$ddf, sitename, date, gpp_mod_ORG = mod)) %>% 
  left_join(dplyr::select(out_eval_BRC$gpp$fluxnet2015$data$ddf, sitename, date, gpp_mod_BRC = mod)) %>% 
  left_join(dplyr::select(obs_eval_itpl$ddf, date, sitename, fapar_itpl = fapar, ppfd_fluxnet2015, temp_day_fluxnet2015, vpd_day_fluxnet2015, prec_fluxnet2015, patm_fluxnet2015), by = c("sitename", "date")) %>% 
  left_join(dplyr::select(obs_eval_spl$ddf, date, sitename, fapar_spl = fapar), by = c("sitename", "date"))

ddf %>% 
  write_csv(path = "./data_share/ddf_fluxnet2015_pmodel_with_forcings_stocker19gmd.csv")

# test
ddf %>% 
  filter(sitename == "CH-Lae") %>% 
  ggplot() +
  geom_line(aes(date, fapar_itpl)) +
  geom_line(aes(date, fapar_spl), color = "red")
```
