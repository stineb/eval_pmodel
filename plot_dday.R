plot_dday <- function( df_dday_agg, df_dday_agg2=NA, df_dday_agg3=NA, bias=FALSE, before, after, filn=NA ){
  ...
  if (!is.na(filn)) pdf( filn, width=6, height=5 )
    ...
    ## Plot bias
    ##--------------------------------------------------------
    par( las=1, mar=c(4,4,2,2), mgp=c(2.7,1,0), xpd=FALSE, xaxs="i", yaxs="i" )
    if (bias){
    ...
    plot( c(-before,after), ylim, 
      type="n", xlab="Days after drought onset", 
      ylab=expression( paste( "Bias (g C m"^{-1}, " d"^{-1}, ")")), 
      axes=FALSE, 
      xlim=xlim,
      ylim=ylim
      )
