...
siteinfo <- rsofun::metainfo_Tier1_sites_kgclimate_fluxnet2015 %>% 
  ...
  ## test
  dplyr::filter(sitename %in% c("FR-Pue", "IT-Cpz", "IT-Noe")) %>% 
...
calibsites <- c("FR-Pue", "IT-Cpz", "IT-Noe")
...
### Evaluate (quickly)
```{r}
mylist <- readr::read_csv("./myselect_fluxnet2015.csv") %>% dplyr::filter( use==1 ) %>% dplyr::pull( Site )
settings_eval <- list(
