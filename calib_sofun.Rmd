...
calibsites <- readr::read_csv( "~/data/flue/flue_stocker18nphyt.csv" ) %>%
              ...
              dplyr::filter( !(site %in% c("DE-Akm", "IT-Ro1"))) %>%  # excluded because fapar data could not be downloaded (WEIRD)
              dplyr::filter(!(site %in% c("AU-ASM", "AU-Wom"))) %>%  # excluded because no GPP data was found in FLUXNET file
              dplyr::filter( site != "FI-Sod" ) %>%  # excluded because some temperature data is missing
              dplyr::select( site ) %>%
              distinct() %>%
