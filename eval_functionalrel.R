...
#' 
eval_functionalrel <- function( df, filnam_params, overwrite = FALSE, ndays_agg = 10, ... ){
  ...
  if (!is.null(ndays_agg)){
    ...
    df_agg <- df %>%  mutate( inbin = cut( date, breaks = breaks, right = FALSE ) ) %>%
      ...
      summarise( 
        ...
        lue_mod_mean = mean( lue_mod, na.rm = TRUE ),
      ...
      mutate( 
        ...
        lue_mod = ifelse(is.nan(lue_mod), NA, lue_mod ),
  ...
  df_training <- dplyr::filter( df_agg, !is.na(lue_obs) & !is.na(lue_mod) & !is.na(temp) & !is.na(vpd) & !is.na(soilm) )
  ...
  ##-------------------------------------
  ## temperature
  eval_response_byvar( df_training, gam, gam_mod, evalvar = "temp", predictors = c("temp", "vpd", "soilm"), kphio = params_opt$kphio, varmin = 0, varmax = 40, nsample = 12, ylim=c(0,0.5), ... ) 

  ## vpd
  eval_response_byvar( df_training, gam, gam_mod, evalvar = "vpd", predictors = c("temp", "vpd", "soilm"), kphio = params_opt$kphio, varmin = 0, varmax = 3000, nsample = 12, ylim=c(0,0.5), ... )

  ## soilm
  eval_response_byvar( df_training, gam, gam_mod, evalvar = "soilm", predictors = c("temp", "vpd", "soilm"), kphio = params_opt$kphio, varmin = 0, varmax = 1.0, nsample = 12, ylim=c(0,0.5), ... )
...
eval_response_byvar <- function( df, gam, gam_mod, evalvar, predictors, kphio, varmin, varmax, nsample, makepdf=FALSE, ... ){
  ...
  predicted_eval_mod <- predict( gam_mod, evaldata )
  evaldata <- evaldata %>% mutate( lue_gam = predicted_eval, lue_mod_gam = predicted_eval_mod )
  ...
  eval_sum <- evaldata %>% group_by_( evalvar ) %>%
                           summarise( 
                                      median_gam = median( lue_gam ),  
                                      q33_gam = quantile( lue_gam, 0.33 ),
                                      q66_gam = quantile( lue_gam, 0.66 ),
                                      q25_gam = quantile( lue_gam, 0.25 ),
                                      q75_gam = quantile( lue_gam, 0.75 ),

                                      median_mod_gam = median( lue_mod_gam ),  
                                      q33_mod_gam = quantile( lue_mod_gam, 0.33 ),
                                      q66_mod_gam = quantile( lue_mod_gam, 0.66 ),
                                      q25_mod_gam = quantile( lue_mod_gam, 0.25 ),
                                      q75_mod_gam = quantile( lue_mod_gam, 0.75 ),
  ...
  if (makepdf) pdf( filn )
    ...
    lines( eval_sum[[evalvar]], eval_sum$median_mod_gam, col="royalblue" )
    polygon( c(eval_sum[[evalvar]], rev(eval_sum[[evalvar]])), c(eval_sum$q33_mod_gam, rev(eval_sum$q66_mod_gam)), col=add_alpha("royalblue", 0.2), border = NA )
