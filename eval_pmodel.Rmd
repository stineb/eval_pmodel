...
settings_eval <- list(
  ...
  path_fluxnet2015_w = "/alphadata01/bstocker/data/FLUXNET-2015_Tier1/20160128/point-scale_none_7d/original/unpacked/",
  ...
  path_gepisat_d     = "/alphadata01/bstocker/data/gepisat/v3_fluxnet2015/daily_gpp/"
  )
##------------------------------------------
...
settings_sims$path_output    = "/alphadata01/bstocker/sofun/output_fluxnet2015_sofun/201/"
settings_sims$path_output_nc = "/alphadata01/bstocker/sofun/output_nc_fluxnet2015_sofun/201/"
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
## Define new output directory and specific simulation parameters and re-run the setup step
settings_sims$path_output    = "/alphadata01/bstocker/sofun/output_fluxnet2015_sofun/s202/"
settings_sims$path_output_nc = "/alphadata01/bstocker/sofun/output_nc_fluxnet2015_sofun/s202/"
settings_sims$soilmstress = FALSE
...
if (!file.exists("data/out_eval_FULL_DT.Rdata")){
...
if (!file.exists("data/out_eval_FULL_NTsub.Rdata")){
...
if (!file.exists("data/out_eval_FULL_Ty.Rdata")){
...
```{r}
plot_linmod <- function( df, sitename, fitted, ... ){
  pdf( paste0("fig/spatial_annual_bysite/plot_spatial_annual_", sitename, ".pdf"), width=5, height=5 )
  par(las=1)
  ...
  df_fitted <- tibble( gpp_mod=df$gpp_mod, fitted=fitted ) %>%
    arrange( gpp_mod )
  lines( df_fitted$gpp_mod, df_fitted$fitted, col="red" )
...
purrr::map( 
  as.list(seq(nrow(out_eval_FULL$data$adf_stats))),
  ~plot_linmod( 
    out_eval_FULL$data$adf_stats$data[.][[1]], 
    xlim = c(0,4000), ylim = c(0,4000),
    sitename = out_eval_FULL$data$adf_stats$sitename[.],
    fitted = out_eval_FULL$data$adf_stats$linmod[.][[1]]$fitted.values
    )
)
```
...
modobs_anomalies_daily_ORG  <- plot_modobs_anomalies_daily( out_eval_ORG,  makepdf=TRUE, label = "ORG", heat=TRUE, greyscale=TRUE )
modobs_anomalies_daily_BRC  <- plot_modobs_anomalies_daily( out_eval_BRC,  makepdf=TRUE, label = "BRC", heat=TRUE, greyscale=TRUE )
modobs_anomalies_daily_FULL <- plot_modobs_anomalies_daily( out_eval_FULL, makepdf=TRUE, label = "FULL", heat=TRUE, greyscale=TRUE )
modobs_anomalies_daily_NULL <- plot_modobs_anomalies_daily( out_eval_NULL, makepdf=TRUE, label = "NULL", heat=TRUE, greyscale=TRUE )

modobs_anomalies_annual_ORG  <- plot_modobs_anomalies_annual( out_eval_ORG,  makepdf=TRUE, label = "ORG", heat=TRUE, greyscale=TRUE )
modobs_anomalies_annual_BRC  <- plot_modobs_anomalies_annual( out_eval_BRC,  makepdf=TRUE, label = "BRC", heat=TRUE, greyscale=TRUE )
modobs_anomalies_annual_FULL <- plot_modobs_anomalies_annual( out_eval_FULL, makepdf=TRUE, label = "FULL", heat=TRUE, greyscale=TRUE )
modobs_anomalies_annual_NULL <- plot_modobs_anomalies_annual( out_eval_NULL, makepdf=TRUE, label = "NULL", heat=TRUE, greyscale=TRUE )
```
...
save(dflist_eval_lue_obs_NT, file = "data/dflist_eval_lue_obs_NT.Rdata")
save(dflist_eval_lue_obs_DT, file = "data/dflist_eval_lue_obs_DT.Rdata")
save(dflist_eval_lue_obs_Ty, file = "data/dflist_eval_lue_obs_Ty.Rdata")
save(dflist_eval_lue_mod, file = "data/dflist_eval_lue_mod.Rdata")
...
plot_functionalrel( 
  df  = dflist_eval_lue_obs_NT[["temp"]], 
  df2 = dflist_eval_lue_obs_DT[["temp"]], 
  df3 = dflist_eval_lue_obs_Ty[["temp"]], 
  df4 = dflist_eval_lue_mod[["temp"]],
...
plot_functionalrel( 
  df  = dflist_eval_lue_obs_NT[["vpd"]], 
  df2 = dflist_eval_lue_obs_DT[["vpd"]], 
  df3 = dflist_eval_lue_obs_Ty[["vpd"]], 
  df4 = dflist_eval_lue_mod[["vpd"]],
...
plot_functionalrel( 
  df  = dflist_eval_lue_obs_NT[["soilm"]], 
  df2 = dflist_eval_lue_obs_DT[["soilm"]], 
  df3 = dflist_eval_lue_obs_Ty[["soilm"]], 
  df4 = dflist_eval_lue_mod[["soilm"]],
...
df_dday_agg_BRC <- eval_droughtresponse( 
  mod = mod_BRC$daily, 
  obs = obs_eval_NT$ddf, 
  path_flue = "/alphadata01/bstocker/data/fLUE/flue_stocker18nphyt.csv",
  before=20, 
  after=79,
  leng_threshold = 10, 
  nbins=10, 
