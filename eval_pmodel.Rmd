...
library(cowplot)
plot_grid(gg_FULL, gg_NULL, gg_FULL_EVI, labels = "auto", label_size = 12)
  
filn <- paste0( dir_figs, "modobs_spatial_annual.pdf" )
ggsave(filn, width=10, height=10)  
```
...
getname_climatezone <- function(climatezone){
  load("./data/df_rosetta.Rdata")
  df_rosetta %>% 
    filter(climatezone==climatezone) %>% 
    select(label) %>% 
    unlist()
}

## combine data
df_meandoy_byclim <- out_eval_FULL$data$meandoydf_byclim_stats$data %>% 
  bind_rows() %>%
  mutate( mod_mean = obs_mean) %>% 
  mutate(setup = "Observed") %>% 
  bind_rows(
    out_eval_NULL$data$meandoydf_byclim_stats$data %>% 
    bind_rows() %>% 
    mutate(setup = "NULL")
  ) %>% 
  bind_rows(
    out_eval_ORG$data$meandoydf_byclim_stats$data %>% 
    bind_rows() %>% 
    mutate(setup = "ORG")
  ) %>% 
  bind_rows(
    out_eval_BRC$data$meandoydf_byclim_stats$data %>% 
    bind_rows() %>% 
    mutate(setup = "BRC")
  ) %>% 
  bind_rows(
    out_eval_FULL$data$meandoydf_byclim_stats$data %>% 
    bind_rows() %>% 
    mutate(setup = "FULL")
  ) %>% 
...
df_meandoy_byclim$setup <- factor(df_meandoy_byclim$setup, levels = c("NULL", "ORG", "BRC", "Observed", "FULL"))
...
df_meandoy_byclim %>% 
  ...
  labs(y = expression( paste("Simulated GPP (g C m"^-2, " d"^-1, ")" ) ), 
       x = "DOY") +
  facet_wrap( ~climatezone, labeller = labeller(climatezone = getname_climatezone) ) +
  theme_gray() +
  theme(legend.position = "bottom") +
  scale_color_manual(
    name="Setup: ", 
    # values=c("black", "red", "royalblue", "#E69F00", "darkseagreen3")
    values=c("Observed" = "black",  "FULL" = "red", "BRC" = "royalblue", "ORG" = "#E69F00", "NULL" = "darkseagreen3")
    ) 
...
## Figure 3
  ...
  library(cowplot)
  plot_grid(gg_FULL, gg_NULL, labels = "auto", label_size = 12)
...
df_meandoy_byclim <- out_eval_FULL$data$meandoydf_byclim_stats$data %>% 
  ...
  bind_rows(
    out_eval_FULL_FPARitp$data$meandoydf_byclim_stats$data %>% 
    bind_rows() %>% 
    mutate(setup = "FULL_FPARitp")
  ) %>% 
  bind_rows(
    out_eval_FULL_EVI$data$meandoydf_byclim_stats$data %>% 
    bind_rows() %>% 
