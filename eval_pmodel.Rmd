...
if (!exists("mod_NULL")) load("data/old/mod_NULL.Rdata")
if (!exists("mod_ORG")) load("data/old/mod_ORG.Rdata")
...
## evaluate NULL simulations
if (!file.exists("data/out_eval_NULLpft.RdataXXX")){
...
### Temperature-dependent quantum yield efficiency (`BRC`)
...
### Full simulations, including soil moisture and temperature stress (`FULL`)
...
if (!exists("out_eval_FULL")) load("data/out_eval_FULL.Rdata")
if (!exists("out_eval_NULL")) load("data/out_eval_NULL.Rdata")
if (!exists("out_eval_NULLpft")) load("data/out_eval_NULL.Rdata")
```
...
table_rsq <- bind_rows( 
  bind_cols( Setup = "ORG",  dplyr::select( getrow_statstable( out_eval_ORG, stat = "rsq" ),  xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "BRC",  dplyr::select( getrow_statstable( out_eval_BRC, stat = "rsq" ),  xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "FULL", dplyr::select( getrow_statstable( out_eval_FULL, stat = "rsq" ), xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "NULL", dplyr::select( getrow_statstable( out_eval_NULL, stat = "rsq" ), xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "NULLpft", dplyr::select( getrow_statstable( out_eval_NULLpft, stat = "rsq" ), xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var") 
  ) %>% 
create_table_latex( caption = "Caption text", filn = "table_rsq.tex" )

table_rmse <- bind_rows( 
  bind_cols( Setup = "ORG",  dplyr::select( getrow_statstable( out_eval_ORG, stat = "rmse" ),  xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "BRC",  dplyr::select( getrow_statstable( out_eval_BRC, stat = "rmse" ),  xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "FULL", dplyr::select( getrow_statstable( out_eval_FULL, stat = "rmse" ), xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "NULL", dplyr::select( getrow_statstable( out_eval_NULL, stat = "rmse" ), xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var"),
  bind_cols( Setup = "NULLpft", dplyr::select( getrow_statstable( out_eval_NULLpft, stat = "rmse" ), xdaily, spatial, annual, seasonal, daily_var ) ) %>%
    rename_( "5-daily"="xdaily", "Spatial"="spatial", "Annual"="annual", "Seasonal"="seasonal", "var(daily)"="daily_var")
  ) %>% 
...
plot_by_doy_allzones( out_eval_FULL, out_eval_BRC, out_eval_ORG, makepdf=TRUE, lab1="FULL", lab2="BRC", lab3="ORG", label="FULL" )
plot_by_doy_allzones( out_eval_NULL, makepdf=TRUE, label="NULL", lab1="NULL" )
