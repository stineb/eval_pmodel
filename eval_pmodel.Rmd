...
write_csv(dplyr::select(out_eval_FULL$data$ddf, sitename, date, gpp_mod), path = "data_public/ddf_FULL.csv")
...
write_csv(dplyr::select(out_eval_FULL_DT$data$ddf, sitename, date, gpp_mod), path = "data_public/ddf_FULL_DT.csv")
write_csv(dplyr::select(out_eval_FULL_NTsub$data$ddf, sitename, date, gpp_mod), path = "data_public/ddf_FULL_NTsub.csv")
write_csv(dplyr::select(out_eval_FULL_Ty$data$ddf, sitename, date, gpp_mod), path = "data_public/ddf_FULL_Ty.csv")
```
...
gg_ddf <- ddf %>% 
  ...
  annotate( 
    ...
    label = paste("sigma[obs] == ", 
                  format( sd(out_eval_FULL$data$idvdf$gpp_obs, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
  annotate( 
    "text", x = 5, y = 0.76, 
    label = paste("sigma[mod] == ", 
                  format( sd(out_eval_FULL$data$idvdf$gpp_mod, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
...
gg_xdf <- xdf %>% 
  ...
  annotate( 
    "text", x = 4, y = 0.8, 
    label = paste("sigma[obs] == ", 
                  format( sd(out_eval_FULL$data$ixvdf$gpp_obs, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
  annotate( 
    "text", x = 4, y = 0.76, 
    label = paste("sigma[mod] == ", 
                  format( sd(out_eval_FULL$data$ixvdf$gpp_mod, na.rm = TRUE), digits = 3)), 
    parse = TRUE, hjust = 0) +
  xlim(-10,10) + ylim(0, 0.8) +
...
## MEAN SEASONALITY
## NT
gg_NT <- out_eval_FULL_NTsub$data$meandoydf_byclim_stats$data %>% 
  bind_rows() %>% 
...
gg_DT <- out_eval_FULL_DT$data$meandoydf_byclim_stats$data %>% 
  bind_rows() %>% 
  filter(climatezone == "Cfb north") %>% 
  ggplot() +
  geom_ribbon(aes(x = doy, ymin = obs_min, ymax = obs_max), fill = "black", alpha = 0.3) +
...
gg_Ty <- out_eval_FULL_Ty$data$meandoydf_byclim_stats$data %>% 
  bind_rows() %>% 
  filter(climatezone == "Cfb north") %>% 
  ggplot() +
  geom_ribbon(aes(x = doy, ymin = obs_min, ymax = obs_max), fill = "black", alpha = 0.3) +
...
## SCATTER PLOT
  ...
  modobs_xdf_NT <- analyse_modobs2(
    out_eval_FULL_NTsub$data$xdf,
    "gpp_mod",
    "gpp_obs",
    type = "heat"
    )
  modobs_xdf_DT <- analyse_modobs2(
    out_eval_FULL_DT$data$xdf,
    "gpp_mod",
    "gpp_obs",
    type = "heat"
    )  
  modobs_xdf_Ty <- analyse_modobs2(
    out_eval_FULL_Ty$data$xdf,
    "gpp_mod",
    "gpp_obs",
    type = "heat"
    )  
