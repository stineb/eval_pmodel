...
missing_mod <- purrr::map_lgl( mod_ORG$daily, ~identical(., NA ) ) %>% which() %>% names()
...
## get sites for which no model output is available and overwrite settings_eval$sitenames
missing_mod <- purrr::map_lgl( mod_full_FPARitp$daily, ~identical(., NA ) ) %>% which() %>% names()
settings_eval$sitenames <- settings_eval$sitenames[which(!(settings_eval$sitenames %in% missing_mod))]
...
## get sites for which no model output is available and overwrite settings_eval$sitenames
missing_mod <- purrr::map_lgl( mod_FULL_EVI$daily, ~identical(., NA ) ) %>% which() %>% names()
settings_eval$sitenames <- settings_eval$sitenames[which(!(settings_eval$sitenames %in% missing_mod))]
...
## get sites for which no model output is available and overwrite settings_eval$sitenames
missing_mod <- purrr::map_lgl( mod_FULL_DT$daily, ~identical(., NA ) ) %>% which() %>% names()
settings_eval$sitenames <- settings_eval$sitenames[which(!(settings_eval$sitenames %in% missing_mod))]
...
if (!file.exists("data/out_eval_ORG_DT.Rdata")){
  out_eval_ORG_DT <- eval_sofun( mod_FULL_DT, settings_eval, settings_sims, obs_eval = obs_eval_DT, overwrite = TRUE )
  save( out_eval_ORG_DT, file = "data/out_eval_ORG_DT.Rdata" )
} else {
  load("data/out_eval_ORG_DT.Rdata")
}

row_rsqtable_FULL_DT <- bind_cols( Setup = "FULL_DT", getrow_statstable( out_eval_ORG_DT, stat = "rsq" ) )
```
...
if (!file.exists("data/mod_RED_Ty.Rdata")){
  ...
  ## update files containing parameter values read by model
  params_opt <- readr::read_csv( "calib_results/params_opt_RED_Ty.csv" )
  nothing <- update_params( params_opt, settings_sims$dir_sofun )
...
if (!file.exists("data/out_eval_ORG_Ty.Rdata")){
  out_eval_ORG_Ty <- eval_sofun( mod_RED_Ty, settings_eval, settings_sims, obs_eval = obs_eval_Ty, overwrite = TRUE )
  save( out_eval_ORG_Ty, file = "data/out_eval_ORG_Ty.Rdata" )
} else {
  load("data/out_eval_ORG_Ty.Rdata")
}

row_rsqtable_RED_Ty <- bind_cols( Setup = "RED_Ty", getrow_statstable( out_eval_ORG_Ty, stat = "rsq" ) )
```
...
```{r}
table_rsq <- rbind( row_rsqtable_NULL, row_rsqtable_NULLpft, row_rsqtable_RED, row_rsqtable_FULL, row_rsqtable_full_FPARitp, row_rsqtable_FULL_EVI, row_rsqtable_FULL_DT, row_rsqtable_RED_Ty )
...
plot_by_doy_allzones( out_eval_ORG, makepdf=TRUE, pattern="RED" ) # out_eval_FULL, out_eval_FULL_EVI, 
plot_by_doy_allzones( out_eval_ORG, makepdf=TRUE, pattern="FULL" ) # out_eval_FULL, out_eval_FULL_EVI, 
...
modobs_mdf <- plot_modobs_monthly( out_eval_ORG, makepdf=TRUE, xlim=c(0,20), ylim=c(0,20), pattern = "RED" )
...
modobs_anomalies_daily_RED  <- plot_modobs_anomalies_daily( out_eval_ORG,  makepdf=TRUE, pattern = "RED" )
...
testdf <- eval_response( out_eval_ORG$data$ddf, "params/params_opt_RED.csv", overwrite = FALSE, ndays_agg = 10, makepdf=FALSE, dir = "./fig/", pattern = "RED"  )
```
...
source("plot_dday.R")
df_dday_agg_RED <- eval_droughtresponse( 
  mod = mod_RED$daily, 
  obs = obs_eval_NT$ddf, 
...
plot_dday( df_dday_agg_RED, df_dday_agg_FULL, before=20, after=79, bias=TRUE )

## An example: Something is going very wrong here.
df_red  <- filter(mod_RED$daily$`AU-How`,  year(date) %in% 2005:2010 )
df_full <- filter(mod_FULL$daily$`AU-How`, year(date) %in% 2005:2010 )
with( df_red, plot(  date, gpp, type="l" ) )
with( df_full, lines( date, gpp, col="red" ) )
plot( df_red$date, df_full$gpp/df_red$gpp, type="l")
```
