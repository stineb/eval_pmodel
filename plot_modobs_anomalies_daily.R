...
##------------------------------------------------------------
plot_modobs_anomalies_daily <- function( out_eval, pattern="", makepdf = FALSE ){
  dir_figs <- "./fig/"
  if (makepdf && !dir.exists(dir_figs)) system( paste0( "mkdir -p ", dir_figs))
  if (makepdf) filn <- paste0( dir_figs, "/modobs_anomalies_daily_", pattern, ".pdf" )
  if (makepdf) print( paste( "Plotting to file:", filn ) )
  if (makepdf) pdf( filn )
    modobs_anomalies_daily <- with( out_eval$data$idvdf, analyse_modobs(
      gpp_mod,
      gpp_obs,
      col=rgb(0,0,0,0.05),
  ...
  ##------------------------------------------------------------
  if (makepdf && !dir.exists(dir_figs)) system( paste0( "mkdir -p ", dir_figs))
  if (makepdf) filn <- paste0( dir_figs, "/hist_anomalies_daily_", pattern, ".pdf" )
  if (makepdf) print( paste( "Plotting to file:", filn ) )
  if (makepdf) pdf( filn )
    par(las=1)
    out <- with( out_eval$data$idvdf, hist( gpp_obs, breaks = 50, col = rgb(0,0,0,0.3), freq = FALSE, main = "Daily anomalies", ylim = c(0,0.6), xlab = expression( paste("GPP anomaly (gC m"^-2, "d"^-1, ")" ) ) ) )
