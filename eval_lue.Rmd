...
load("data/v3/out_eval_ORG.Rdata")
load("data/v3/out_eval_FULL.Rdata")
mc <- 12.0107  # molecular mass of carbon

ddf_FULL <- out_eval_FULL$data$ddf %>% 
...
mdf_FULL <- ddf_FULL %>%
  ...
  right_join(out_eval_FULL$data$mdf, by = c("sitename", "year", "moy"))
...
mdf_ORG <- ddf_ORG %>%
  ...
  summarise(lue_obs = mean(lue_obs, na.rm=TRUE),
            lue_mod = mean(lue_mod, na.rm=TRUE)) %>% 
  right_join(out_eval_ORG$data$mdf, by = c("sitename", "year", "moy"))
...
<!--   summarise(lue_obs = mean(lue_obs, na.rm=TRUE), -->
<!--             lue_mod = mean(lue_mod, na.rm=TRUE)) %>%  -->
<!--   right_join(out_eval_FULL$data$xdf, by = c("sitename", "year", "moy")) -->
...
adf_FULL <- ddf_FULL %>%   
  ...
  dplyr::filter(apar > 0.1) %>% 
  right_join(out_eval_FULL$data$adf, by=c("sitename", "year")) %>% 
  mutate(lue_obs = gpp_obs / (mc * apar),
         lue_mod = gpp_mod / (mc * apar) ) %>% 
  ungroup()  %>% 
  left_join(rsofun::metainfo_Tier1_sites_kgclimate_fluxnet2015 %>% select(sitename, classid), 
            by="sitename") %>% 
  mutate(lue_obs = remove_outliers(lue_obs),
         lue_mod = remove_outliers(lue_mod))
...
adf_ORG <- ddf_ORG %>%   
  ...
  summarise( apar = sum(apar)) %>% 
  dplyr::filter(apar > 0.1) %>% 
  right_join(out_eval_ORG$data$adf, by=c("sitename", "year")) %>% 
  mutate(lue_obs = gpp_obs / (mc * apar),
         lue_mod = gpp_mod / (mc * apar) ) %>% 
  ungroup()  %>% 
  left_join(rsofun::metainfo_Tier1_sites_kgclimate_fluxnet2015 %>% select(sitename, classid), 
...
out_adf_ORG <- adf_ORG %>% rbeni::analyse_modobs2("lue_mod", "lue_obs", xlab = expression(paste("Modelled LUE (mol mol" ^{-1}, ")")), ylab = expression(paste("Observed LUE (mol mol" ^{-1}, ")"))) # , filnam = "fig/modobs_lue_annual.pdf"
...
```{r}
load("data/linmod_NULL.RData")
lue_fit <- coef(linmod)
load("data/v3/out_eval_NULL.Rdata")

ddf_FULL_test <- ddf_FULL %>% 
  sample_frac(0.1) %>% 
  mutate(gpp_mod_reNULL = lue_fit * fapar * ppfd_fluxnet2015) %>% 
  left_join( select(out_eval_NULL$data$ddf, sitename, date, gpp_mod_NULL = gpp_mod), by = c("sitename", "date")) %>% 
